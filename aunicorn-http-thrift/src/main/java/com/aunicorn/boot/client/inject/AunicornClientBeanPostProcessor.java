...
import com.aunicorn.boot.client.stubfactory.StubFactory;
...
public class AunicornClientBeanPostProcessor  implements BeanPostProcessor {
    ...
    private THttpClientFactory tHttpClientFactory = null;
    ...
    protected <T> T processInjectionPoint(final Member injectionTarget, final Class<T> injectionType, final AunicornClient annotation) {
        ...
        try {
            TTransport transport = getTHttpClientFactory().getTransport(url);
    ...
    /**
     * Lazy getter for the {@link THttpClientFactory}.
     *
     * @return The THttpClient  factory to use.
     */
    private THttpClientFactory getTHttpClientFactory(){
        if (this.tHttpClientFactory == null) {
            final THttpClientFactory factory = this.applicationContext.getBean(THttpClientFactory.class);
